...
model_vgg = torch.hub.load('pytorch/vision:v0.10.0', 'vgg19', pretrained=True)
model_vgg.classifier[6] = nn.Linear(model_vgg.classifier[6].in_features, 2)
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model_res = model_vgg.to(device)
...
class_weights = get_class_weights(df['label'])
...
train_model(model_vgg, train_loader, val_loader, criterion, optimizer, num_epochs=15)
...
with open("vgg_labels_data.pkl", "wb") as f:
